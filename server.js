...
async function fearGreed(){
 ...
 const r=await fetch(FG_URL,{headers:{'User-Agent':'Mozilla/5.0 StockDashboard/2.0','Accept':'application/json'}});
 if(!r.ok) throw new Error(`fear greed ${r.status}`);
 const j=await r.json();
 const values=Array.isArray(j?.values)?j.values:[]; const dates=Array.isArray(j?.dates)?j.dates:[];
 const raw=values.at(-1); const value=Number(raw); const asOf=j?.asOf||dates.at(-1)||null;
 if(!Number.isFinite(value)) throw new Error('fear greed value unavailable');
 const rating=value<20?'Extreme Fear':value<45?'Fear':value<55?'Neutral':value<80?'Greed':'Extreme Greed';
 const out={available:true,value,rating,asOf,source:'Fear & Greed Graph',refresh:'hourly'}; cache.set(key,{t:Date.now(),v:out}); return out;
}
...
app.get('/api/dashboard/:symbol',async(req,res)=>{try{const sym=req.params.symbol.trim().toUpperCase(); if(!/^[A-Z0-9.^=-]{1,15}$/.test(sym))return res.status(400).json({error:'Invalid ticker'}); const [s,sp,v,fgResult]=await Promise.all([chart(sym),chart('^GSPC'),chart('^VIX','1y'),fearGreed().catch(e=>({available:false,note:e.message}))]); const sm=metrics(s), spm=metrics(sp), vm=metrics(v); res.json({stock:sm,market:{vix:vm.price,vixAsOf:vm.priceAsOf,sp500Drawdown:spm.drawdown,sp500Price:spm.price,sp500DailyClose:spm.dailyClose,sp500Sma200:spm.sma200,sp500Above200:spm.above200,sp500Distance200:spm.distance200,sp500DailyAsOf:spm.dailyAsOf},fearGreed:fgResult,sourceNote:'가격/기술지표는 무료 시장 데이터로 계산되며 지연·중단될 수 있습니다.'});}catch(e){res.status(502).json({error:'데이터를 불러오지 못했습니다.',detail:e.message})}});
